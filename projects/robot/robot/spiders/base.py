...
class RobotSpider(scrapy.Spider):
    ...
    def baidu_rpc_request(self, data, furl=""):
        ...
        return scrapy.Request(url=server_url, callback=self.baidu_rpc_response, method="POST", body=data_post,
                              headers={"Content-Type": 'application/json'}, meta={"baidu_rpc": data, "furl": furl})

    def baidu_rpc_response(self, response):

        if response.status / 100 != 2:
            self.log("Baidu_RPC %s http_error http_code:%d" % (response.url, response.status),
                     level=scrapy.log.CRITICAL)
            return
        res = json.loads(response.body)
        if res["err_no"] != 0:
            self.log("Baidu_RPC %s rpc_error rpc_code:%d %s" % (
            response.url, res["err_no"], response.meta["baidu_rpc"]["url"]), level=scrapy.log.CRITICAL)
        else:
            self.log("Baidu_RPC %s %s ok" % (response.meta["furl"], response.meta["baidu_rpc"]["url"]),
